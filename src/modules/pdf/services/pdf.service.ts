...
export class PdfService implements IPdfService {
  ...
  async generate(dto: GeneratePdfDto): Promise<Buffer> {
    try {
      const pdfVO = new PdfDocumentValueObject(dto.title, dto.width, dto.height);
  ...
  getFileInfo(buffer: Buffer): PdfFileInfo {
    ...
    return { size, sizeFormatted, mimeType: PDF_CONSTANTS.MIME_TYPE };
  }
  ...
  async generateAndSaveMerchantCard(data: MerchantCardData, fileName?: string): Promise<{ buffer: Buffer; filePath: string; info: PdfFileInfo }> {
