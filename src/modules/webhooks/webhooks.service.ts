...
export class WebhooksService {
	...
	private async checkIdempotency(source: WebhookSource, key: string): Promise<WebhookLog | null> {
		...
		const existing = await this.webhookRepo.findOne({ where: { source, idempotency_key: key, received_at: MoreThan(windowStart) }, order: { received_at: 'DESC' } as any });
	...
	private async updateWebhookLog(log: WebhookLog, result: { success: boolean; error?: string; data?: any }): Promise<void> {
		if (result.success) {
			log.markProcessed();
		} else {
	...
	async getWebhookLogs(page = 1, limit = 10, source?: WebhookSource, status?: WebhookStatus): Promise<{ data: WebhookLog[]; total: number }> {
		...
		const [data, total] = await qb.orderBy('log.received_at', 'DESC').skip((page - 1) * limit).take(limit).getManyAndCount();
		return { data, total };
	}
